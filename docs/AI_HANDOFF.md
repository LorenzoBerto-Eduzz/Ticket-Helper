...
- Version `1.9.6` builds on the `1.9` stabilization model, the `1.9.1` HubSpot email-gather reliability work, the `1.9.2` producer warnings, the `1.9.4`/`1.9.5` internal shortcut/config UI work, and the session-only Historico popup/history navigation UI.
