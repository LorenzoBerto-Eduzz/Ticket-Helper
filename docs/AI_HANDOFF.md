...
- Version `1.9.4` builds on the `1.9` stabilization model, the `1.9.1` HubSpot email-gather reliability work, and the `1.9.2` producer warnings.
...
- Default Chrome command copy shortcuts: `Alt+1` ticket/chat ID, `Alt+2` doc, `Alt+3` email, `Alt+4` first/name. Chrome only allows four default suggested command shortcuts, so extra shortcuts use TicketHelper internal page-level handling. `Alt+5` is the first internal shortcut, stored in `chrome.storage.local` as `internalShortcuts`, defaults to opening the options/config page, and is editable/disableable on the options page. Internal shortcuts only run on supported pages while the extension is enabled and ignore input/textarea/select/contenteditable targets.
