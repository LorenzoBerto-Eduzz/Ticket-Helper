...
- Producer warnings: the options page has `Avisos de produtores` stored in `chrome.storage.local` under `producerWarningRules`. Each rule has a normalized unique producer/seller string and warning text. `content.js` also runs on `bo.eduzz.com` and starts only a lightweight Faturas popup watcher there; it scans visible Faturas rows only when the popup/table exists, matches the seller name in the Produto column by normalized exact match, and inserts a body-level fixed light-red overlay below the seller email line so it does not affect table/popup layout. The overlay clamps to two lines with ellipsis, expands on hover, and must keep normal text-selection behavior (`cursor: text`, selectable text) so users can copy warning text. Do not rewrite/reposition an existing overlay while the user is actively selecting text inside it. Keep this BO watcher isolated from ticket extraction/popup logic and debounced to avoid BO performance issues.
